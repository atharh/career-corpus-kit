---
# This block is strict YAML, read by tools as well as people. A prose item — anything with a
# colon in it, or opening with `*` or a quote — goes in a block scalar (`- |`), as the two
# below show. The trap: `- foo: bar` does not fail, it parses as a one-key mapping and the
# sentence silently stops being a value.
...
technologies:        # what this arc evidences the user doing, or being asked about —
                     # `[TECHNOLOGIES-DECLARED]`. Canonical lowercase slugs, hyphenated
                     # (`kubernetes`, not `k8s`; aliases live in a capability file's
                     # `covers:`). Written at capture, grown when a later round surfaces
                     # one; a withdrawn technology comes off. `[]` when the arc evidences none.
  - <technology>
