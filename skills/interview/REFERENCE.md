...
**1. State the worst reading and make them rebut it.** `[WORST-READING]` The highest-yield technique by far.
...
**2. Cross-check the arithmetic — but only where it changes a claim.** `[CHECK-ARITHMETIC]` Dates, durations, and
...
**6. Chase the generic word.** `[GENERIC-WORD]` When the résumé says the abstract thing — "improving
...
**9. Offer a wrong theory, invite rejection.** `[WRONG-THEORY]` A concrete reading they can push against beats
